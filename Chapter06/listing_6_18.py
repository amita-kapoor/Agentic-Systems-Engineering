...
def generator(task: dict) -> DraftReport:
    ...
    parts.append("\nDraft the report now.")  #E

    raw = llm_call(GENERATOR_SYSTEM, "\n".join(parts), schema={"type": "object"})  #F
    try:
        clauses = [ReportClause(**c) for c in raw.get("clauses", [])]  #G
        return DraftReport(
        ...
        return DraftReport(summary=f"[parse_error: {e}]", clauses=[], open_issues=[])  #H
