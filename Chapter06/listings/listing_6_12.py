def production_reflect(task, generator, planner, tools, critic, memory, rules=None, max_iters=3):
    if max_iters < 1:
        raise ValueError("max_iters must be at least 1")  # Ensure valid iteration count
    ...
    # Initial plan
    plan = planner(context)  # Generate initial plan
    ...
    for _ in range(max_iters):
        ...
        )  # Execute plan to produce output
        ...
        )  # Evaluate using combined feedback sources
        ...
        )  # Revise plan based on critique (double-loop)
